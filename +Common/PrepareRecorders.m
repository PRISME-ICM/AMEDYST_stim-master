...
switch S.Task
    ...
    case {'ADAPT_Reward','ADAPT_Punishment'}
        ...
        OutRecorder = SampleRecorder( { 'BlockNumber' 'Trial index', 'jitter duration (s)', 'Deviation (°)', 'Value[0-100]', 'Target angle (°)', 'frame_start', 'frame_stop', 'Reaction time OUT (ms)', 'Travel time OUT (ms)', 'Error Angle', 'Points', 'failedTrial'} , 5000 );
...
SR = SampleRecorder( { 'time (s)', 'X (pixels)', 'Y (pixels)', 'R (pixels)', 'Theta (°)' } ,...
    round(EP.Data{end,2}*S.PTB.FPS*1.20) ); % ( duration of the task +20% )
