        ...
        switch EP.Data{evt,1}
            ...
            case {'Direct_Pre', 'Deviation', 'Direct_Post'} % --------------------------------
                ...
                while stepActionRunning
                    ...
                    % KND:
                    disp('Distance to BigCircle')
                    disp(abs(Cursor.R - BigCircle.diameter/2))

                    if abs(Cursor.R - BigCircle.diameter/2) < 5 && ADAPT.IsInCircle( PrevCursor, BigCircle )
                        % is cursor out BigCircle
                        BigCircle.frameCurrentColor = Green;
                        %disp('OK ici %%%%%%%%%%%%%%%%%')
                        %pause
                    else
                ...
                % KND: Export trajectory data
                disp('export data')
                trajectory_to_CSV(SR,EP, 'trajectory.csv');
