...
%     end % switch
    ...
    for  trial = 1 : NrTrials
        ...
        % KND : Save SIGNED AUC
        s.Trials(trial).signed_auc         = signed_auc;
        s.signed_auc(trial) = signed_auc;

        % KND: Peak Velocity
        [  s.Trials(trial).peak_Velocity.value ,  s.Trials(trial).peak_Velocity.sample ] = max(sum(diff(xy,1,1).^2,2));
    ...
    fprintf('mean signed AUC in block ''%s'' = %g pixels² \n', name, round(mean(s.signed_auc,'omitnan')))
