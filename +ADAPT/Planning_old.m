...
% Create and prepare
header = { ...
    'event_name', 'onset (s)', 'duration (s)', 'Block #', 'Trial #', 'Deviation (°)', 'Target angle (°)', ...
    'Variable pause duration (s)'  ...
    'Probability of reward (%)', 'is rewarded (0/1)' ...
    'Premotor baseline (s)'};
...
for block = 1 : size(Paradigm,1)
    ...
    for trial_idx_in_block = 1 : Paradigm{block,3}
        ...
        EP.AddPlanning({ ...
            Paradigm{block,1} NextOnset(EP) trialDuration block trial_counter Paradigm{block,2}  Parameters.TargetAngles(angleList(end)) ... % 1 -7 
            pauseJitter ...
            value double(rand*100<value) ...
            PausePreMotorJitter});
