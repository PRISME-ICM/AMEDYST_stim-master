...
header = { 'event_name', 'onset(s)', 'duration(s)', 'Block#', 'Trial#', 'Deviation(°)', 'Target angle (°)', 'Variable pause duration (s)', 'RewardPunishment'};
...
%     
    ...
    for trial_idx_in_block = 1 : Paradigm{block,3}
        ...
        EP.AddPlanning({ Paradigm{block,1} NextOnset(EP) trialDuration block trial_counter Paradigm{block,2}  Parameters.TargetAngles(angleList(end)) pauseJitter Paradigm{block,4}});  % value double(rand*100<value)
